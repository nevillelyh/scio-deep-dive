...
object WordCount13 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cTag = new TupleTag[Long]()
    val uTag = new TupleTag[String]()
    val grouped = KeyedPCollectionTuple
      .of(cTag, wordCount.toKV.internal)
      .and(uTag, that.toKV.internal)
      .apply(CoGroupByKey.create())
    val joined = new SCollection(grouped)
      .flatMap { kv =>
        val k = kv.getKey
        val lhs = kv.getValue.getAll(cTag).asScala
        val rhs = kv.getValue.getAll(uTag).asScala
        for (l <- lhs; r <- rhs) yield (k, (l, r))
      }

    val result = joined.map(kv => kv._1 + " " + kv._2._1 + " " + kv._2._2)
